...
export default async function handler(
  ...
  try {
    ...
    console.log("📅 PW API todays-schedule response keys:", Object.keys(response.data || {}));
    console.log("📅 PW API todays-schedule response.data?.data type:", typeof response.data?.data, "isArray:", Array.isArray(response.data?.data), "length:", response.data?.data?.length);
