...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Without a database configured (local preview), cleanup is a no-op.
  if (await dbConnect() === null) {
    return res.status(200).json({ status: "skipped", reason: "no database configured" });
  }
