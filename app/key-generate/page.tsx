...
export default async function KeyGeneratePage({ searchParams }: { searchParams?: Promise<{ [key: string]: string }> }) {
  ...
  const anon_id = params.anon_id || (await cookies()).get("anon_id")?.value || "";
  ...
  if (anon_id) {
    ...
    if (verification) {
      ...
      // Check if already verified for this batch and verification has not expired (unless force/reverify is true)
      const isForce = params.force === "true" || params.reverify === "true";
 ...
 const verifyUrl = `${baseUrl}/verify?token=${encodeURIComponent(token)}`;
  ...
  return (
    <KeyGenerateClient
      ...
      rawVerifyUrl={token ? `${process.env.NODE_ENV === "production" ? "https" : "http"}://${(await headers()).get("host")}/verify?token=${encodeURIComponent(token)}` : ""}
    />
  );
} 
