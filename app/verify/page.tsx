...
export default async function VerifyPage({
  ...
  // Check if there is an existing active expireAt for this batch to stack/add 24 hours
  const verifiedBatch = verification.verifiedBatch || [];
  const existingSubdoc = verifiedBatch.find((vb: any) => vb.batchId === payload.batchid);
  const nowDate = new Date();

  let baseTime = nowDate.getTime();
  if (existingSubdoc && existingSubdoc.expireAt) {
    const existingExpire = new Date(existingSubdoc.expireAt).getTime();
    if (existingExpire > baseTime) {
      baseTime = existingExpire; // Add 24 hours on top of remaining time
    }
  }

  const expireAt = new Date(baseTime + 24 * 60 * 60 * 1000); // Add 24 hours

  // Remove any existing verification entry for this batch first
  await Verification.updateOne(
    { anon_id: payload.anon_id },
    { $pull: { verifiedBatch: { batchId: payload.batchid } } }
  );

  // Insert new stacked verification for this batch
  const newVerified = {
