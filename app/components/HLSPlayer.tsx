...
const VideoPlayer: React.FC<Props> = ({ baseUrl, signedQuery, attachments, downloadUrl, lectureTitle, lectureThumbnail, onError }) => {
  const hlsRef = useRef<Hls | null>(null);

  const videoRef = useRef<HTMLVideoElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  const LIVE_LATENCY = 30; // 30 seconds delay threshold for live sync
  ...
  const [isStreamLive, setIsStreamLive] = useState(false); // true only for actual live streams
  ...
  const goToLive = () => {
    ...
    const seekToLiveEdge = () => {
      const liveEdge = video.seekable.end(video.seekable.length - 1);
      const targetTime = Math.max(0, liveEdge - 25); // Target 25 seconds behind absolute live edge for safe buffering
  ...
  useEffect(() => {
    ...
    // Links coming from the lecture stream API are already proxied and
    // CORS-enabled, and they carry a signed policy in the query string.
    // Touching them (re-proxying or appending params) breaks the signature
    // and the CDN answers 403 — so they are passed through untouched.
    const isPreSignedStream = (targetUrl: string) =>
      targetUrl.includes("/api/v/") ||
      targetUrl.includes("herokuapp.com") ||
      targetUrl.includes("workers.dev") ||
      targetUrl.includes("testwave.cc") ||
      targetUrl.includes("bunny-cdn") ||
      targetUrl.includes("Key-Pair-Id") ||
      targetUrl.includes("Signature=");

    const proxyUrlIfNeeded = (targetUrl: string) => {
      if (targetUrl.startsWith("/api/proxy") || targetUrl.includes("/api/proxy?url=")) {
        return targetUrl;
      }
      if (isPreSignedStream(targetUrl)) {
        return targetUrl;
      }
      // pimaxer.in: CORS enabled, play directly without proxy
      if (targetUrl.includes("pimaxer.in")) {
        return targetUrl;
      }
      if (
        targetUrl.includes("cloudfront.net") ||
        targetUrl.includes("penpencil.co") ||
        targetUrl.includes("classplus") ||
        targetUrl.includes("pw.live")
      ) {
    ...
    const hls = new Hls({
      xhrSetup: (xhr, url) => {
        let finalUrl = url;
        const extraQuery = (signedQuery || "").replace(/^\?/, "");
        if (extraQuery && !isPreSignedStream(url)) {
          try {
            const reqUrl = new URL(url, window.location.origin);
            const queryParams = new URLSearchParams(extraQuery);
            ...
            finalUrl = `${url}${sep}${extraQuery}`;
          }
        }
        xhr.open("GET", proxyUrlIfNeeded(finalUrl), true);
      },
      maxBufferLength: 30,
    ...
    hlsRef.current = hls;
    hls.loadSource(proxyUrlIfNeeded(baseUrl));
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, () => {
      ...
      // Begin playback as soon as HLS is ready; browsers that block autoplay
      // still leave the standard play control available.
      void video.play().catch(() => {});
    });

    // Detect live vs VOD from the actual playlist details
    hls.on(Hls.Events.LEVEL_LOADED, (_event: any, data: any) => {
      setIsStreamLive(!!data.details?.live);
    });

    hls.on(Hls.Events.ERROR, (event, data) => {
      console.error("HLS.js error:", data);
      const isDrmOrKeyError = data.details && (
        data.details.toLowerCase().includes("key") ||
        data.details.toLowerCase().includes("decrypt") ||
        data.details.toLowerCase().includes("drm")
      );
      if (data.fatal || isDrmOrKeyError) {
        setIsBuffering(false);
        setPlaybackError(
          data.details?.toLowerCase().includes("codec") ||
            data.details?.toLowerCase().includes("bufferappend")
            ? "This browser cannot decode the stream format. Try opening the lecture in Chrome or Safari."
            : "The stream could not be played. Check your connection and retry."
        );
        if (onError) {
          onError(data);
        }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (video.seekable?.length > 0) {
        ...
        const latency = liveEdge - video.currentTime;
        // Only track live edge position for actual live streams
        if (isStreamLive) {
                  ...
                  {downloadUrl && (
                    <div
                      onClick={() => {
                        ...
                        const deferredPrompt = (window as any).deferredAppPrompt;
                        if (deferredPrompt) {
                          ...
                          deferredPrompt.userChoice.then((choiceResult: any) => {
                            ...
                            (window as any).deferredAppPrompt = null;
                          });
                        }

                        const downloaderUrl = process.env.NEXT_PUBLIC_DOWNLOADER_URL || 'http://localhost:5000';
                        
                        // Pre-open a blank tab to prevent browser popup blockers from intercepting async redirect
                        const newWindow = window.open('', '_blank');
                        if (!newWindow) {
                          alert("Please allow popups to download this video.");
                          return;
                        }
                        ...
                        newWindow.document.write(`
                          <html>
                            <head>
                              <title>Preparing Secure Download</title>
                              <style>
                                ...
                                .loader { border: 4px solid #1e293b; border-top: 4px solid #6366f1; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto 16px; }
                                @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
                                .text { font-size: 16px; font-weight: 500; text-align: center; }
                              </style>
                            </head>
                            <body>
                              <div>
                                <div class="loader"></div>
                                <div class="text">Generating secure download link...</div>
                              </div>
                            </body>
                          </html>
                        `);

                        let formattedDownloadUrl = downloadUrl || '';
                        const uuidMatch = formattedDownloadUrl.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i);
                        if (uuidMatch) {
                          formattedDownloadUrl = `https://stream.pimaxer.in/${uuidMatch[1]}/master.m3u8`;
                        }
                        ...
                        fetch('/api/download-token', {
                          ...
                          body: JSON.stringify({
                            videoUrl: formattedDownloadUrl,
                            lectureName: lectureTitle || 'Lecture',
                        ...
                        .then(data => {
                          if (data.success && data.tokenId) {
                            newWindow.location.href = `${downloaderUrl}?token=${data.tokenId}`;
                          } else {
                            newWindow.document.body.innerHTML = `<div style="text-align:center; color:#ef4444; font-family:sans-serif; background:#0b0f19; padding: 20px;"><h3>Failed to prepare download</h3><p>${data.message || 'Unknown error occurred'}</p></div>`;
                            setTimeout(() => newWindow.close(), 4000);
                          }
                        })
                        .catch(err => {
                          newWindow.document.body.innerHTML = `<div style="text-align:center; color:#ef4444; font-family:sans-serif; background:#0b0f19; padding: 20px;"><h3>Failed to connect to server</h3><p>${err.message || 'Please check your connection'}</p></div>`;
                          setTimeout(() => newWindow.close(), 4000);
                        });
                      }}
                      className="flex items-center cursor-pointer w-full p-2.5 gap-2.5 border-t border-white/10 mt-1 hover:bg-[#252830] rounded transition-colors"
                    >
                      <div className="w-6 h-6">
                        <HugeiconsIcon
                          icon={Download04Icon}
                      ...
                      <div className="w-full">
                        <div className="font-semibold text-sm">
                          <span className="text-white">Save to Device</span>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    {isStreamLive ? (
                      <div
                        className={`text-sm p-1 rounded-md flex items-center text-center flex-row gap-1 cursor-pointer ${
                          isLive ? "bg-red-500" : "border border-red-500 text-white"
                        }`}
                        title="Go Live"
                        onClick={goToLive}
                      >
                      ...
                      <div className="vjs-time-control vjs-time-control vjs-control text-sm opacity-80">
                        {formatTime(duration)}
                      </div>
                    ) : null}
                  </div>
          ...
          <div className="player bg-black">
            ...
            <video
              ...
              onError={(e) => {
                ...
                setIsBuffering(false);
                setPlaybackError("The video could not be decoded by this browser.");
                if (onError) {
                  onError(e);
                }
              }}
            />
            {playbackError && (
              <div className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-3 bg-black px-6 text-center text-white">
                <span className="max-w-md text-sm">{playbackError}</span>
                <button
                  type="button"
            ...
            {isBuffering && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-30 pointer-events-none transition-all duration-300">
                <div className="flex flex-col items-center gap-3">
                  <HugeiconsIcon
                    icon={ReloadIcon}
                    className="w-12 h-12 text-white animate-spin drop-shadow-lg"
                  />
                  <span className="text-white text-xs font-extrabold tracking-wider bg-black/55 px-2.5 py-1 rounded-full drop-shadow-md">
                    Loading Video...
                  </span>
                </div>
              </div>
            ...
            <div className="max-h-[80vh] overflow-auto">
              <div className="flex flex-wrap w-full">
                ...
                <div className="notes w-full pb-3">
                  <div className="p-3 md:p-5 lg:p-5 flex flex-col gap-3">
                    {attachments && attachments.map((att: any, idx: number) => (
                      ...
                      >
                        <div className="pd flex flex-row justify-between items-center">
                          ...
                          <div className="he px-2 text-xs text-gray-800 w-full truncate">
                            {att.name || att.homeworkName || `Attachment ${idx + 1}`}
                          </div>
