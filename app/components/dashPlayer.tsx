...
// @ts-ignore - shaka-player ships a non-module type declaration
...
const VideoPlayer: React.FC<Props> = ({
  ...
  const preprocessClearKeys = (clearKeys?: Record<string, string>) => {
    if (!clearKeys) return undefined;
    const keys = { ...clearKeys };
    const firstKeyVal = Object.values(keys)[0];
    if (firstKeyVal) {
      // Ensure the standard PenPencil default KID is mapped to the decryption key
      keys["7211165ec6609edc247a70daaccc2878"] = firstKeyVal;
    }
    return keys;
  };
  ...
  useEffect(() => {
    ...
    const shakaInitTimeout = setTimeout(() => {
      ...
      // Detect if this is a pimaxer.in URL (direct, no-proxy path)
      const isPimaxerDirect = src.includes("pimaxer.in") && !src.includes("/api/proxy");
      const isCorsProxy = src.includes("cors.pimaxer.in");

      // Extract query parameters from src or signedUrlQuery
      ...
      // For pimaxer.in: load directly (no proxy).
      // For all other CDN URLs: proxy through /api/proxy as before.
      const proxyUri = (uri: string) => {
        if (uri.startsWith("/api/proxy") || uri.includes("/api/proxy?url=")) {
          return uri;
        }
        // pimaxer.in (direct path): no proxying needed
        if (uri.includes("pimaxer.in") && isPimaxerDirect) return uri;
        ...
        if (
          finalUri.includes("cloudfront.net") ||
          finalUri.includes("penpencil.co") ||
          finalUri.includes("classplus") ||
          finalUri.includes("pimaxer.in") ||
          finalUri.includes("pw.live")
        ) {
            ...
            try {
              ...
              newPlayer.configure({
                drm: {
                  ...
                  clearKeys: preprocessClearKeys(drmConfig.clearKeys),
                },
                streaming: {
                  bufferingGoal: 30,
                  rebufferingGoal: 2,
              ...
              const loadUrl = (isCorsProxy || isPimaxerDirect) ? src : proxyUri(src);
              await newPlayer.load(loadUrl, savedTime, mimeType);
        ...
        try {
          player.configure({
           ...
           player.getNetworkingEngine()?.registerRequestFilter((requestType: number, request: shaka.extern.Request) => {
            ...
            if (isCorsProxy) {
              // cors.pimaxer.in path: rewrite sec1.pw.live → cors.pimaxer.in for segment requests
              // and append signed query params (Policy, Key-Pair-Id, Signature) to each segment
              request.uris = request.uris.map((uri: string) => {
                let u = uri
                  .replace("sec1.pw.live", "cors.pimaxer.in")
                  .replace(/([a-z0-9-]+\.pw\.live)/i, "cors.pimaxer.in");
                // Append signed token to segment requests only if not already present
                if (effectiveQuery && !u.includes("Policy=") && !u.includes("policy=")) {
                  const sep = u.includes("?") ? "&" : "?";
                  u += sep + effectiveQuery.replace(/^\?/, "");
                }
                return u;
              });
            } else if (isPimaxerDirect) {
              // Direct stream.pimaxer.in stream: load directly, no proxy, no rewriting
            } else {
              request.uris = request.uris.map(proxyUri);
            }
          });
          ...
          const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
          const mimeType = src.includes(".m3u8") ? "application/x-mpegurl" : "application/dash+xml";
          // For cors.pimaxer.in and direct pimaxer: load the manifest directly
          const loadUrl = (isCorsProxy || isPimaxerDirect) ? src : proxyUri(src);
          await player.load(loadUrl, savedTime, mimeType);
          ...
          qualities.sort((a: Quality, b: Quality) => a.height - b.height);
  ...
  const handleSpeedChange = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Dynamically scale buffer for high playback speeds to prevent stalling
    const player = playerRef.current;
    if (player) {
      const scaledBufferGoal = Math.ceil(300 * Math.max(value, 1));
      const scaledPrefetch = value >= 2 ? 8 : 5;
      player.configure({
        streaming: {
          bufferingGoal: scaledBufferGoal,
          segmentPrefetchLimit: scaledPrefetch,
        },
      });
    }
  };
                ...
                <div className="bg-[#1B2124] flex flex-col  rounded-md w-[200px] p-3 shadow-xl border border-gray-800">
                  ...
                  <div
                    onClick={() => {
                      ...
                      const downloaderUrl = process.env.NEXT_PUBLIC_DOWNLOADER_URL || 'http://localhost:4500';
                    ...
                    className="flex items-center cursor-pointer w-full p-2.5 gap-2.5 hover:bg-[#252830] rounded transition-colors"
                  >
                    <div className="w-6 h-6">
                      <HugeiconsIcon
                        icon={Download04Icon}
