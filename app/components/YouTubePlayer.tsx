...
export default function YouTubePlayer({ videoId, ContentId }: Props) {
  ...
  const [availableQualities, setAvailableQualities] = useState<
    { id: number; height: number; bandwidth: number; label?: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setCurrentTime(time);
      if (ContentId && time > 0) {
        localStorage.setItem(`lectureTime_${ContentId}`, time.toString());
      }
    }, 500);

    return () => clearInterval(interval);
  }, [playerRef.current, ContentId]);
            ...
            <YouTube
              ...
              opts={{
                ...
                playerVars: {
                  ...
                  start: ContentId ? Math.floor(parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0")) : 0,
                },
              }}
              onReady={(event) => {
