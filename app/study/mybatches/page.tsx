...
import { getEnrolledBatches, BatchInfo, SourceBatchInfo } from "@/utils/api";
import { DEFAULT_BATCH_BANNER } from "@/lib/branding";
      ...
      try {
        ...
        const details = await Promise.all(
          enrolled.map(async (batch: any) => {
            try {
              const src = await SourceBatchInfo(batch.batchId).catch(() => null);
              if (src?.data) {
                return {
                  ...src.data,
                  _id: batch._id,
                  batchId: batch.batchId,
                  batchName: batch.name || src.data.batchName,
                  batchImage: src.data.batchImage || DEFAULT_BATCH_BANNER,
                };
              }
              ...
              return {
                ...
                batchImage: info.iosPreviewImageUrl || (info.previewImage?.baseUrl && info.previewImage?.key ? info.previewImage.baseUrl + info.previewImage.key : DEFAULT_BATCH_BANNER),
