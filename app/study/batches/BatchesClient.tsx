...
export function formatDate(dateStr: string): string {
  if (!dateStr || typeof dateStr !== "string") return "Date unavailable";

  const trimmed = dateStr.trim();
  const isoDate = parseISO(trimmed);
  if (isValid(isoDate)) return format(isoDate, "dd MMM yyyy");

  for (const pattern of ["dd MMM yyyy", "d MMM yyyy", "dd MMMM yyyy", "d MMMM yyyy"]) {
...
export default function BatchesClient() {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="glass-card shadow-spring-sm border border-spring-leaf/10 dark:border-spring-mint/10 !rounded-2xl p-6">
        <h1 className="text-2xl font-semibold mb-4 text-spring-forest dark:text-spring-forest-dark">Batches</h1>
        ...
        >
          <div className="relative w-full">
            <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
              <Search strokeWidth={3} className="w-4 h-4 text-spring-leaf dark:text-spring-mint" />
            </div>
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="flex-grow block w-full p-4 ps-10 text-sm divshadow border border-spring-leaf/15 dark:border-spring-mint/20 rounded-xl focus:border-spring-leaf dark:focus:border-spring-mint focus:outline-none outline-none bg-white/80 dark:bg-spring-forest/60 text-spring-forest dark:text-spring-forest-dark focus:ring-1 focus:ring-spring-leaf"
              placeholder="Search Your Batch"
            />
            <button
              type="submit"
              className="absolute end-2.5 bottom-2.5 font-semibold rounded-lg text-sm px-4 py-2 bg-gradient-to-r from-spring-mint to-spring-leaf dark:from-spring-mint-dark dark:to-spring-leaf-dark text-white hover:scale-[1.02] active:scale-95 transition-all duration-300 shadow-spring-sm hover:shadow-spring-md"
            >
              Search
            </button>
          </div>
